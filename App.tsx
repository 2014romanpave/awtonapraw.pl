...
const SERVICE_SLUGS: Record<string, string> = {
  diag: '/diagnostyka-komputerowa',
  engine: '/naprawa-silnika',
  oil: '/serwis-olejowy',
  brakes: '/uklad-hamulcowy',
  gbo: '/serwis-lpg',
  electric: '/elektryka-samochodowa',
};

const SLUG_TO_ID: Record<string, string> = Object.fromEntries(
  Object.entries(SERVICE_SLUGS).map(([id, slug]) => [slug.replace('/', ''), id])
);
...
const TRANSLATIONS: Record<Lang, TranslationSet> = {
  UA: {
    ...
    features: {
      ...
      items: [
        { title: 'Глибока діагностика', desc: 'Використовуємо ліцензійне обладнання. Ми знаємо, як знайти причину, а не просто видалити помилку.', icon: 'precision_manufacturing' },
...
const SERVICES_DATA: ServiceDetails[] = [
  { id: 'diag', icon: 'biotech', image: DiagnosticsImage, ua: "Комп'ютерна діагностика", pl: 'Diagnostyka komputerowa', price: '120 zł', description_ua: "Професійна комп'ютерна діагностика авто за допомогою ліцензійного обладнання. Швидке виявлення несправностей електронних систем, двигуна, КПП та інших вузлів.", description_pl: 'Profesjonalna diagnostyka komputerowa przy użyciu licencjonowanego sprzętu. Szybkie wykrywanie usterek systemów elektronicznych, silnika i skrzyni biegów.' },
  { id: 'engine', icon: 'settings_input_component', image: EngineImage, ua: 'Ремонт двигуна та ГРМ', pl: 'Naprawa silnika i rozrządu', price: 'від 600 zł', description_ua: 'Капітальний та поточний ремонт двигунів будь-якої складності. Заміна ГРМ, ремонт головок блоку, заміна поршневих груп.', description_pl: 'Remonty kapitalne i bieżące silników o dowolnej złożoności. Wymiana rozrządu, naprawa głowic cylindrów, wymiana tłoków.' },
  { id: 'oil', icon: 'water_drop', image: OilImage, ua: 'Регламентне ТО', pl: 'Serwis olejowy / Przegląd', price: 'від 100 zł', description_ua: 'Повне технічне обслуговування авто: заміна оливи, фільтрів, гальмівної рідини, охолоджуючої рідини. Дотримуємось регламентів виробника.', description_pl: 'Kompleksowy przegląd techniczny: wymiana oleju, filtrów, płynu hamulcowego, chłodniczego. Przestrzegamy zaleceń producenta.' },
  { id: 'brakes', icon: 'stop_circle', image: BrakesImage, ua: 'Гальмівна система', pl: 'Układ hamulcowy', price: 'від 150 zł', description_ua: 'Діагностика та ремонт гальмівної системи: заміна колодок, дисків, супортів, гальмівних циліндрів. Гарантуємо безпеку та якість.', description_pl: 'Diagnostyka i naprawa układu hamulcowego: wymiana klocków, tarcz, zacisków, cylinderków hamulcowych. Gwarantujemy bezpieczeństwo.' },
  { id: 'gbo', icon: 'propane_tank', image: GboImage, ua: 'Сервіс та налаштування ГБО', pl: 'Serwis i regulacja LPG', price: 'від 120 zł', description_ua: 'Встановлення, налаштування та сервіс газобалонного обладнання. Оптимізація витрати палива, діагностика газових систем.', description_pl: 'Montaż, regulacja i serwis instalacji gazowych. Optymalizacja zużycia paliwa, diagnostyka systemów gazowych.' },
  { id: 'electric', icon: 'flash_on', image: ElectricImage, ua: 'Автоелектрика', pl: 'Elektryka samochodowa', price: 'від 150 zł', description_ua: 'Ремонт електрообладнання авто: стартерів, генераторів, проводки, панелі приладів, освітлення. Діагностика електросхем.', description_pl: 'Naprawa instalacji elektrycznej: rozruszników, alternatorów, okablowania, deski rozdzielczej, oświetlenia. Diagnostyka schematów.' },
];
...
const BookingPopup: React.FC<{ isOpen: boolean; onClose: () => void; lang: Lang; initialService?: string }> = ({ isOpen, onClose, lang, initialService }) => {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', service: initialService || t.options[0] });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSending(true);
    const text = `🔧 *Новий запит з сайту awtonapraw.pl*\n\n👤 *Імя:* ${formData.name}\n📞 *Телефон:* ${formData.phone}\n🛠 *Послуга:* ${formData.service}`;
    try {
      ...
      <div className="relative bg-white dark:bg-apple-dark w-full max-w-lg rounded-apple-2xl p-8 shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-1.5">
            ...
            <input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full bg-gray-50 dark:bg-apple-dark border border-gray-100 dark:border-white/10 rounded-xl p-4 dark:text-white outline-none focus:ring-2 focus:ring-primary" placeholder={t.name} />
          </div>
          <div className="space-y-1.5">
          ...
          {submitted ? (
            ...
            <button type="submit" disabled={sending} className="w-full bg-primary text-white py-5 rounded-xl font-bold text-lg shadow-xl shadow-primary/30 hover:scale-[1.01] active:scale-95 transition-all mt-4 flex items-center justify-center gap-3 disabled:opacity-60 disabled:scale-100">
              <span className="material-symbols-outlined">{sending ? 'hourglass_empty' : 'send'}</span>
              {sending ? (lang === 'UA' ? 'Відправка...' : 'Wysyłanie...') : t.submit}
            </button>
          )}
        </form>
      </div>
...
const Navbar: React.FC<{ lang: Lang; setLang: (l: Lang) => void; theme: Theme; toggleTheme: () => void }> = ({ lang, setLang, theme, toggleTheme }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] glass border-b border-gray-200/40 dark:border-white/5 h-[64px] flex items-center">
      <div className="max-w-[1200px] mx-auto w-full px-6 flex items-center justify-between">
        <button onClick={() => { navigate('/'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex items-center gap-3 group">
        ...
        <nav className="hidden lg:flex items-center gap-10 text-[13px] font-semibold text-apple-dark/70 dark:text-apple-gray/70 uppercase tracking-widest">
          <button onClick={() => { navigate('/'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="hover:text-primary transition-colors uppercase">{t.home}</button>
...
const HomePage: React.FC<{ lang: Lang; openBooking: (s?: string) => void }> = ({ lang, openBooking }) => {
  const t = TRANSLATIONS[lang];
  const navigate = useNavigate();
  const openRoute = () => window.open(GOOGLE_MAPS_ROUTE, '_blank');
    ...
    <>
      <section className="pt-48 pb-24 px-6 max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="animate-in slide-in-from-left-20 duration-1000">
          ...
          <h1 className="text-[42px] md:text-[54px] font-extrabold tracking-tight mb-8 leading-[1.1] dark:text-white text-balance">{t.hero.title}</h1>
          <p className="text-xl md:text-2xl text-gray-500 dark:text-gray-400 mb-12 leading-relaxed max-w-xl font-medium">{t.hero.desc}</p>
          <div className="flex flex-col sm:flex-row gap-6 mb-16">
            <button onClick={() => openBooking()} className="bg-primary text-white px-12 py-5 rounded-2xl text-[18px] font-bold shadow-2xl shadow-primary/30 hover:scale-[1.02] active:scale-95 transition-all">{t.hero.request}</button>
            <a href="tel:+48735088101" className="bg-gray-50 dark:bg-apple-dark text-apple-dark dark:text-white px-12 py-5 rounded-2xl text-[18px] font-bold border border-gray-200 dark:border-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-all text-center">{t.hero.call}</a>
          </div>
          <div className="grid grid-cols-3 gap-8">{t.stats.items.map((s, i) => <StatItem key={i} {...s} />)}</div>
        </div>
        <div className="relative animate-in fade-in zoom-in duration-1000 delay-300">
          ...
          <div className="relative rounded-apple-2xl overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] border-[10px] border-white dark:border-apple-dark aspect-square lg:aspect-[4/5] transform hover:rotate-1 transition-transform duration-700">
            <img src={HeroImage} className="w-full h-full object-cover" alt="Serwis samochodowy Poznań" />
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-32 px-6 max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES_DATA.map((service) => {
            ...
            return (
              <div key={service.id} className="group relative h-[480px] rounded-apple-2xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-700"
                onClick={() => { navigate(SERVICE_SLUGS[service.id]); window.scrollTo({ top: 0, behavior: 'instant' }); }}>
                ...
                <div className="relative h-full flex flex-col p-10 text-white">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold border border-white/10 italic">{service.price}</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="relative">
            ...
            <div className="relative glass p-12 rounded-apple-2xl border border-gray-200/50 dark:border-white/10 shadow-2xl text-center">
              ...
              <button onClick={() => openBooking()} className="w-full bg-primary text-white py-6 rounded-2xl font-bold text-xl shadow-xl shadow-primary/30 hover:scale-[1.02] transition-all">{t.hero.request}</button>
            </div>
          </div>
        </div>
      </section>
...
const ServicePage: React.FC<{ lang: Lang; openBooking: (s?: string) => void }> = ({ lang, openBooking }) => {
  const t = TRANSLATIONS[lang];
  const navigate = useNavigate();
  const location = useLocation();

  const slug = location.pathname.replace('/', '');
  const serviceId = SLUG_TO_ID[slug];
  const service = SERVICES_DATA.find(s => s.id === serviceId);

  useEffect(() => { window.scrollTo({ top: 0, behavior: 'instant' }); }, [location.pathname]);

  if (!service) { navigate('/'); return null; }

  const title = lang === 'UA' ? service.ua : service.pl;
  const description = lang === 'UA' ? service.description_ua : service.description_pl;

  return (
    <div className="pt-48 pb-32 px-6 max-w-[1000px] mx-auto animate-in fade-in slide-in-from-bottom-10 duration-700">
      <button onClick={() => { navigate('/'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="inline-flex items-center gap-2 text-primary font-extrabold mb-12 hover:gap-4 transition-all duration-300 group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
        <div className="rounded-apple-2xl overflow-hidden shadow-2xl relative group">
          <img src={service.image} className="w-full aspect-[4/5] object-cover transition-transform duration-1000 group-hover:scale-105" alt={title} />
          <div className="absolute top-6 left-6 px-4 py-2 bg-black/30 backdrop-blur-xl rounded-full text-white font-bold text-sm italic border border-white/20">{service.price}</div>
        </div>
        <div>
          <h1 className="text-4xl md:text-6xl font-extrabold mb-8 tracking-tighter dark:text-white leading-[1.1]">{title}</h1>
          <p className="text-xl text-gray-500 dark:text-gray-400 leading-relaxed mb-10 font-medium italic text-balance">{description}</p>
          ...
          <button onClick={() => openBooking(title)} className="w-full bg-primary text-white py-5 rounded-xl font-bold text-lg shadow-2xl shadow-primary/30 hover:scale-[1.01] active:scale-95 transition-all">{t.hero.request}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="max-w-[1200px] mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center text-center md:text-left mb-16">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 leading-relaxed max-w-sm mx-auto md:mx-0 font-medium italic">
              {lang === 'UA' ? 'Преміальна якість сервісу у самому серці Познані. Працюємо на результат та вашу безпеку.' : 'Serwis jakości premium w samym sercu Poznania. Pracujemy na efekt i Twoje bezpieczeństwo.'}
            </p>
          </div>
        ...
        <div className="pt-12 border-t border-gray-200/50 dark:border-white/5 text-center">
          <p className="text-[13px] font-medium text-gray-400 dark:text-gray-500">
            ...
            <a href="https://highfalutin-receipt-d7a.notion.site/UX-UI-Designer-5ba1936438ad4bc7beaaf9e8dff8c7e5" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-bold transition-all">Pavel Roman</a>
          </p>
        </div>
      </div>
    </footer>
...
const AppInner: React.FC = () => {
  ...
  const openBooking = (serviceName?: string) => { setSelectedService(serviceName); setIsBookingOpen(true); };

  return (
    <div className="min-h-screen selection:bg-primary selection:text-white overflow-x-hidden">
      <Navbar lang={lang} setLang={setLang} theme={theme} toggleTheme={() => setTheme(t => t === 'light' ? 'dark' : 'light')} />
      ...
      <main className="transition-opacity duration-700">
        <Routes>
          <Route path="/" element={<HomePage lang={lang} openBooking={openBooking} />} />
          <Route path="/diagnostyka-komputerowa" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="/naprawa-silnika" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="/serwis-olejowy" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="/uklad-hamulcowy" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="/serwis-lpg" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="/elektryka-samochodowa" element={<ServicePage lang={lang} openBooking={openBooking} />} />
          <Route path="*" element={<HomePage lang={lang} openBooking={openBooking} />} />
        </Routes>
      </main>
      <Footer lang={lang} />
