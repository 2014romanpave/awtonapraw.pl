...
const TRANSLATIONS: Record<Lang, TranslationSet> = {
  UA: {
    ...
    services: {
      ...
      desc: 'Комплексне обслуговування: від точної компютерної діагностики до складного ремонту,',
      more: 'Детальніше',
    },
    form: {
      title: 'Ми вам зателефонуємо.',
      desc: 'Залиште ваші контакти, і майстер звяжеться з вами для уточнення деталей та запису на візит.',
      name: "Ваше ім'я",
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] glass border-b border-gray-200/40 dark:border-white/5 h-[64px] flex items-center">
      <div className="max-w-[1200px] mx-auto w-full px-6 flex items-center justify-between">
        ...
        <button onClick={() => { setPage('home'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="hover:text-primary transition-colors uppercase">{t.home}</button>
